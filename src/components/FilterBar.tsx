...
export const FilterBar = memo(({ filters, onFiltersChange }: FilterBarProps) => {
  ...
  const setRating = (rating: string) => {
    onFiltersChange({ ...filters, rating: rating === 'all' ? undefined : rating });
  };
  ...
  const setScoreRange = (range: string) => {
    if (range === 'all') {
      ...
      const selectedRange = SCORE_RANGES.find((r) => `${r.min}-${r.max}` === range);
      if (selectedRange) {
        onFiltersChange({
          ...filters,
